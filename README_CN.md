...
## 更新
### 2026.6.9
从建图结果来看，FAST-LIO/Faster-LIO 在厂房场景中明显退化，点云没有形成清晰的空间结构，而 LIO-SAM 能够恢复出货架、墙面、立柱等较稳定的厂房结构。结合代码实现分析，我认为这并不是单纯的参数问题，而是前端几何约束模型与场景结构不匹配导致的。

FAST-LIO/Faster-LIO 前端并没有像 LIO-SAM 那样显式区分 corner 特征和 surface 特征，而是对当前帧降采样后的点云逐点遍历，在子图中搜索邻近点，并尝试拟合局部平面，然后构造 point-to-plane 残差。虽然代码中会根据平面拟合误差和残差大小做一定筛选，但这种筛选本质上仍然是判断邻域是否可以被平面近似，并没有进一步判断该局部结构是否更适合作为线特征或边缘特征使用。

在普通办公室、小房间等场景中，四周墙面、地面、柜面等大面积 surface 结构较多，局部平面拟合比较稳定，因此 FAST-LIO/Faster-LIO 的 point-to-plane 约束通常能够提供较好的 scan-to-submap 匹配效果。但在大厂房场景中，激光雷达往往难以稳定观测到完整连续的四周墙面，真正占主导的结构反而是货架、立柱、横梁等 corner/edge/line 特征。如果仍然只依赖平面残差，一方面会丢失大量本应作为 corner/line 使用的有效结构，另一方面也可能把部分非平面结构错误地当作平面使用，导致残差质量下降，最终造成里程计退化和地图发散。

相比之下，LIO-SAM 会先提取 corner 和 surface 两类特征。在 scan-to-map 优化时，corner 特征用于点到线约束，surface 特征用于点到面约束，并且只有几何结构足够明显的特征点才会进入残差矩阵。因此，在货架、立柱、边缘结构丰富的大厂房中，LIO-SAM 能够更充分地利用这些线状和边缘结构来约束位姿，所以建图结果明显优于 FAST-LIO/Faster-LIO。

- 把前端里程计由fastlio 改成了lio-sam ， 不改变原本的接口，只增加了分支，用来解决fastlio 漂移，建图失败
- 这是原本的fastlio 建图的结果，可以看到地图杂乱无章，没有明显结构;
- FAST-LIO 演示
