...
public:
    ...
    PointCloudType::Ptr BuildRawDeskewedCloudForLastFrame()
    {
        PointCloudType::Ptr raw(new PointCloudType());
        ...
        for (const auto& point : laserCloudIn->points)
        {
            ...
            if (rowIdn < 0 || rowIdn >= N_SCAN)
                continue;
            raw->push_back(deskewPoint(&point, point.time, savedDeskewState));
        }

        raw->header.stamp =
            static_cast<std::uint64_t>(std::llround(savedDeskewState.timeScanCur * 1e9));
        raw->header.frame_id = savedFrameId;
        raw->height = 1;
        raw->width = raw->size();
        raw->is_dense = true;
        return raw;
