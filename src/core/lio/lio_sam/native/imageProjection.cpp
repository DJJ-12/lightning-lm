...
public:
    bool hasCloudInfo = false;
    lio_sam::msg::CloudInfo latestCloudInfo;
            ...
            ParamServer("lio_sam_imageProjection", options), deskewFlag(0)
    {
        ...
        else
        {
            RCLCPP_WARN(get_logger(),
                "[MAPPING_MODE] lioMode=mapping. ImageProjection will use IMU rotation only; "
                "mapOptimization will provide low-speed translation initial guess.");
        }
...
#else
        RCLCPP_WARN(get_logger(), "[LIGHTNING_OFFLINE] ImageProjection topic endpoints disabled.");
#endif
    ...
    bool processSyncedCloud(const sensor_msgs::msg::PointCloud2::SharedPtr& laserCloudMsg,
                            const std::vector<sensor_msgs::msg::Imu>& syncedImus)
    {
        hasCloudInfo = false;
        {
            std::lock_guard<std::mutex> lock1(imuLock);
            imuQueue.clear();
            for (const auto& imu : syncedImus)
    ...
    bool cachePointCloud(const sensor_msgs::msg::PointCloud2::SharedPtr& laserCloudMsg)
    {
        ...
        timeScanEnd = timeScanCur + pointTimeLast;
    ...
    void publishClouds()
    {
        ...
        latestCloudInfo = cloudInfo;
        hasCloudInfo = true;
        if (pubLaserCloudInfo)
