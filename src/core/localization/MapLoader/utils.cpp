...
{
    // Check if any of the corners of the box are within the cylinder
    if (box_min_point.x - radius <= center_x && center_x <= box_max_point.x + radius && box_min_point.y - radius <= center_y && center_y <= box_max_point.y + radius)
    {
        return true;
    }
    ...
    return std::hypot(dx0, dy0) <= radius || std::hypot(dx1, dy0) <= radius || std::hypot(dx0, dy1) <= radius || std::hypot(dx1, dy1) <= radius;
}
...
bool is_grid_within_queried_area(const DiffMapReqInfo& area, const PCDFileMetadata metadata)
{
    ...
    bool res = cylinder_and_box_overlap_exists(center_x, center_y, radius, metadata.min, metadata.max);
    return res;
}
