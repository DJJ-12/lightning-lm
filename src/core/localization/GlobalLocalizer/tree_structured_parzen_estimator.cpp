...
TreeStructuredParzenEstimator::TreeStructuredParzenEstimator(const Direction direction, const int64_t n_startup_trials, std::vector<double> sample_mean, std::vector<double> sample_stddev)
    : above_num_(0), direction_(direction), n_startup_trials_(n_startup_trials), input_dimension_(INDEX_NUM), sample_mean_(std::move(sample_mean)), sample_stddev_(std::move(sample_stddev))
{
    if (sample_mean_.size() != INDEX_NUM)
    {
        std::cerr << "sample_mean size is invalid" << std::endl;
        throw std::runtime_error("sample_mean size is invalid");
    }
    if (sample_stddev_.size() != INDEX_NUM)
    {
...
void TreeStructuredParzenEstimator::add_trial(const Trial& trial)
{
    ...
    above_num_ = std::min({static_cast<int64_t>(10), static_cast<int64_t>(static_cast<double>(trials_.size()) * max_good_rate)});
}
...
double TreeStructuredParzenEstimator::log_gaussian_pdf(const Input& input, const Input& mu, const Input& sigma)
{
