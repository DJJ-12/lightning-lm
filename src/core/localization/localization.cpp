...
bool Localization::Init(const std::string& yaml_path, const std::string& global_map_path) {
    ...
    T_base_lidar_matrix_f_ = T_base_lidar.matrix().cast<float>();
    LOG(INFO) << "[BASE_LIDAR] T_base_lidar trans="
              << T_base_lidar.translation().transpose()
              << ", rpy_deg="
...
void Localization::Finish() {
    ...
    LOG(INFO) << "[Localization::Finish] complete";
}
...
void Localization::EmitResult(const LocalizationResult& result) {
    if (result_callback_) {
